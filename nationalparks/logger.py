...
def update_park_image_ids(image_counts):
    '''
    Update park image id log.
    '''
    ## define cyclic name for log
    today = date.today()
    logfile = os.path.join("..", "logs", "update_photo_ids-{}-{}.log".format(today.year, today.month))

    ## write the data to csv file
    header = ['unique_id', 'timestamp']
    header += [key for key in image_counts.keys()]
    ...
    with open(logfile, 'a') as csvfile:
        ...
        if write_header:
            writer.writerow(header)
        to_write = map(str, [uuid.uuid4(),strftime("%Y_%m_%_d_%H_%M_%S")] + [x for x in image_counts.values()])
        writer.writerow(to_write)